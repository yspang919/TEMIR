...
# R library location
# If it is not provided (NA), packages will be installed and read from the default R library path.
r_library_loc = 'C:/Users/Documents/TEMIR/lib/'
...
# Set TEMIR directory:
TEMIR_dir = path.expand('~/Documents/TEMIR/')
# Set source code directory:
code_dir = paste0(TEMIR_dir, 'code_v2.0/')
# Set meteorological data directory:
met_data_dir = paste0(TEMIR_dir, 'TEMIR_inputs/met_data/GEOS_2x2.5/')
...
# For single site simulations, users could provide site measurements data (e.g., FLUXNET2015, other site-measurements) stored in .RData to drive TEMIR instead of mapped inputs (e.g., ozone, meteorology).
# Meteorological maps (MERRA2, GEOS-FP) are required regardless for a complete set of required model variables.
# Refer to 'settings_1site_inputs.R' for additional settings.
use_site_obs_flag = TRUE
...
# Running a single-site simulation?
# If users want to drive TEMIR with single-site meteorological/land surface data (including FLUXNET), please modify settings_1site_inputs.R.
...
if (single_site_flag) {
   # Update: FLUXNET options are now moved to settings_1site_inputs.R
   # Specify location (lon, lat) of local site of interest:
   # crop simulation example: Maize at southern France
   lon_sim = 0.0
   lat_sim = 46.0
} else {
...
# Please refer to each PFT by its PFT number quote in the first column of PFT_df (i.e., not_vegetated = 0)
sim_PFT = 17
...
# Which radiative transfer model? ('two-stream', 'Beer')
# 'two-stream': the default model with two-stream approximation and light extinction by both leaves and stems will be used to calculate variables "phi_sun", "phi_sha", "LAI_sun",  "LAI_sha" and "K_b", among others.
...
if (O3_damage_flag && !O3_fixed_flag) {
   ...
   # O3_data_dir will be overwritten by the one declared in settings_1site_inputs.R if both 'using_site_o3_flag' and 'use_site_obs_flag' are TRUE.
...
# If so (LAI_data_flag=TRUE), the default PFT-level LAI from CLM-SP would be scaled to the user-defined (total, grid-level) LAI so that, when weighted by PFT fractions and summed to obtain grid-level LAI, it would match the user-defined LAI.
LAI_data_flag = FALSE
...
# output_variables = c('A_can', 'R_can', 'g_can',
...
# Additional outputs for ozone simulations
if (O3_damage_flag) {
	output_variables = c(output_variables, 'CUO_can', 'CUO_sun', 'CUO_sha')
}
...
if (O3_damage_flag) {
   available_outputs_df = rbind.data.frame(
      ...
      # Canopy ozone uptake
      c('CUO_can', 'mmol m^-2', 'Cumulative O3 uptake for canopy', 'PFT'),
