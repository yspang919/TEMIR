...
f_maintenance_respiration_fluxes_CLM45 = function(root_frac_array, T_soil_array, T_2M, leaf_N, livestem_N, livecoarseroot_N, fineroot_N, grain_N) {
    ...
    # Root maintenance respiration depends on soil temperature, which can have multiple layers.
    if (length(root_frac_array) == 1 && length(T_soil_array_degC) == 1) {
        # single layer of T soil
        mr_fineroot_total = fineroot_N * mr_perN_20C * q10_mr^ ((T_soil_array_degC - 20) / 10)
    } else {
        # multiple layers of T soil, scale each layer according to the root fraction of each temperature layer
        mr_fineroot_total = 0
        for (layer in 1:(length(T_soil_array_degC)-1)) {
