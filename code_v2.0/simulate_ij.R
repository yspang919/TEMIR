...
f_simulate_ij = function(IJ) {
    ...
    # Redefine new history output for temp_data for crop simulation
    if (biogeochem_flag) {
        BGC_temp_var_vec = c('LAI_PFT_hist_ijd', 'SAI_PFT_hist_ijd', 'htop_PFT_hist_ijd',
                             'leafC_PFT_hist_ijd', 
                             'livestemC_PFT_hist_ijd', 'livestemC_resv_PFT_hist_ijd', 'deadstemC_PFT_hist_ijd', 
                             'finerootC_PFT_hist_ijd', 'livecoarserootC_PFT_hist_ijd', 'deadcoarserootC_PFT_hist_ijd', 
                             'grainC_PFT_hist_ijd',
                             ...
                             'day_of_planting_PFT_hist_ijd', 'day_of_grain_filling_PFT_hist_ijd', 'day_of_harvesting_PFT_hist_ijd', 'day_since_crop_senescence_PFT_hist_ijd',
                             'astem_PFT_hist_ijd', 'aleaf_PFT_hist_ijd', 'astem_leafem_PFT_hist_ijd', 'aleaf_leafem_PFT_hist_ijd')
    ...
    if (biogeochem_flag) {
        if (T_soil_source == 'MERRA2') {
            # Currently only using the shallowest layer of TSOIL for calculating mean soil temperature
            T_soil_daily =  mean(TSOIL1[i,j,], na.rm = TRUE) 
        } else if (T_soil_source == 'custom') {
            stop('Implement your own method to calculate mean soil temperature with custom Tsoil map.')
        }
    }
    ...
    # Soil parameters:
    # Soil parameters from CLM land surface data (default)
    ...
    # Jacky: handle soil albedo later, not relevant to the simulation
    ...
    for (ipft in (sim_PFT + 1)) { # + 1 to shift counter bare land into the index
        ...
        if (!biogeochem_flag) {
            ...
            # Modelled leaf and stem area index (m^2 m^-2)
            if (d == 1) {
                # Initialize values of LAI and SAI from initial_data for the first day of the simulation
                LAI = LAI_initial_map[i,j,ipft]
                SAI = SAI_initial_map[i,j,ipft]
            } else {
                # Reading LAI and SAI of the last modelled day from temp_data
            ...
            # Replacing LAI value from with measurements (single-site single-PFT simulation) for photosynthesis calculation.
            # The output LAI value is still updated at plant_physiology.R
            if (single_site_flag && use_site_obs_flag && using_site_LAI_flag) {
                LAI = LAI_today
            }
        }
        ...
        if ((LAI < 0.01 && !biogeochem_flag) | PFT_frac[i,j,ipft] < PFT_frac_min) {
            ...
            if (single_site_flag && d == 1) {
                print(paste0("Skip calculation for ipft = ", ipft, " - ", pftname[ipft], ' % converage at the grid cell = ', signif(PFT_frac[i,j,ipft]*100, digits = 3)))
                print(paste0('If you want to simulate ', pftname[ipft], ", adjust the variable 'PFT_frac_threshold' in input_TEMIR_crop_extension.R"))
            }
            next
            ...
            # Additional PFT-specific parameters used in crop simulations
            if (biogeochem_flag) {
                ...
                crop_flag = crop[ipft]
                # Managed crops (i.e., excluding C3 generic crops):
                maize_flag = if (any(PFT_df$PFT_description[ipft] == c('corn', 'irrigated_corn'))) 1 else 0
                springwheat_flag = if (any(PFT_df$PFT_description[ipft] == c('spring_temperate_cereal', 'irrigated_spring_temperate_cereal'))) 1 else 0
                winterwheat_flag = if (any(PFT_df$PFT_description[ipft] == c('winter_temperate_cereal', 'irrigated_winter_temperate_cereal'))) 1 else 0
                soybean_flag = if (any(PFT_df$PFT_description[ipft] == c('soybean', 'irrigated_soybean'))) 1 else 0
                managed_crop_flag = maize_flag || springwheat_flag || winterwheat_flag || soybean_flag
                ...
                # Shrub PFT
                shrub_flag = if (any(PFT_df$PFT_description[ipft] == c('broadleaf_evergreen_shrub', 'broadleaf_deciduous_temperate_shrub', 'broadleaf_deciduous_boreal_shrub'))) 1 else 0
                # Base temperature of the accumulation of GDD for crops
                # Tropical maize/soybean: GDD_base_T = 10; 'Tropical' wheat: GDD_base_T = 12 - 0.4 * abs(lat)
                # TODO: double check 'spring_temperate_cereal' == CLM5 tropical wheat parameter-wise? 
                GDD_base_T = baset[ipft]
                if (at_tropical_flag && (maize_flag || soybean_flag)) GDD_base_T = 10
                if (at_tropical_flag && springwheat_flag) GDD_base_T = 12 - 0.4*abs(lat[j])
                ...
                repr_GDDfraction = grnfill[ipft]
                if (at_tropical_flag && (maize_flag || soybean_flag)) repr_GDDfraction = 0.5
                # Maximum GDD_mat allowed for crops
                GDDmat_max = hybgdd[ipft]
                if (at_tropical_flag && soybean_flag) GDDmat_max = 2100
                ...
                # Daily min planting temperature requirement (K)
                min_T_planting_req = min_planting_temp[ipft]
                if (at_tropical_flag && (maize_flag || soybean_flag)) min_T_planting_req = 283.15
                # Maximum growing season length allowed
                crop_season_length_max = mxmat[ipft]
                if (at_tropical_flag && maize_flag) crop_season_length_max = 160
                ...
                # Root fraction for the calculation of maintenance respiration (calculated from soil temperature measurement depth from a map source)
                root_frac_map_Tsoil_pft = if (length(map_TS_meas_depth) > 1) {root_frac_map_Tsoil[ipft,]} else {1}
                # Root fraction calculated from soil temperature measurements depth from a site source
                if (single_site_flag && use_site_obs_flag && using_site_met_flag && any(replace_TS_flag_array)) {
                    # dimension (pft, #soil layer)
                    root_frac_site_Tsoil_pft = if (length(site_TS_meas_depth) > 1) {root_frac_site_Tsoil[ipft,]} else {1}
                }
            ...
            # Reading planting and harvesting GDD requirements (version 2.0)
            # TODO: Current 'prescribed-map' is largely based on the Sack calender, in the future a more generalized script to read in planting date is required.
            if (biogeochem_flag) {
                ...
                if (crop_planting_date_method == 'prescribed-map') {
                    if (maize_flag) {
                        if (crop_growing_season == 'primary') {
                            ...
                            stop("'crop_growing_season' for Sack dataset is not correctly specified")
                        }
            ...
            for (h in 1:(24/dt_hr)) {
                ...
                # Incoming radiation:
                # Incident direct and diffuse beam PAR (W m^-2):
                PAR_beam = PARDR[i,j,h]; PAR_diff = PARDF[i,j,h]
                # If other forms of PAR measurement is available, PAR_beam and PAR_diff are re-calculated (only with single-site custom met. data)
                if (single_site_flag && using_site_met_flag && !FLUXNET_flag) {
                    if (exists('PARTOT') && replaced_PARTOT_flag && !replaced_map_PARDR_flag && !replaced_map_PARDF_flag) {
                        # Only total PAR is measured at the site, direct and diffuse PAR are derived based on the ratio of direct/diffuse PAR in MERRA2
                        # Direct and diffuse PAR are obtained by multiplying measured total PAR by the MERRA2 direct/diffuse partition.
                        par_beam_fraction = if (PARDR[i,j,h] + PARDF[i,j,h] > 0) {(PARDR[i,j,h]) / (PARDR[i,j,h] + PARDF[i,j,h])} else NA
                        PAR_beam = if (!is.na(par_beam_fraction)) {PARTOT[h] * par_beam_fraction} else 0
                        PAR_diff = if (!is.na(par_beam_fraction)) {PARTOT[h] * (1 - par_beam_fraction)} else 0
                    } else if (exists('PARTOT') && replaced_PARTOT_flag && sum(c(replaced_map_PARDR_flag, replaced_map_PARDF_flag)) == 1) {
                        # Total PAR and one of the direct / diffuse PAR is measured
                        # The unmeasured PAR component is the difference of the other two values
                        if (replaced_map_PARDR_flag) {PAR_diff = PARTOT[h] - PARDR[i,j,h]; PAR_beam = PARDR[i,j,h]}
                        if (replaced_map_PARDF_flag) {PAR_beam = PARTOT[h] - PARDF[i,j,h]; PAR_diff = PARDF[i,j,h]}
                    } else {
                        # Both direct and diffuse PAR measurements available, values at grid cell of interest of PARDR and PARDF are already replaced at f_replace_metmaps_with_custom_data().
                        # Or PAR measurements not available, therefore using default met. field values
                        # No action here.
                    }
                }
                ...
                if (single_site_flag && use_site_obs_flag && (FLUXNET_flag || (using_site_met_flag &&replaced_map_WS_flag))) {
                    if (FLUXNET_flag) {u_10m = WS[i,j,h]} else {u_10m = WS[h]}
                } else {
                    # Derived from met. field
                    u_10m = sqrt(U10M[i,j,h]^2 + V10M[i,j,h]^2)
                }
                
                # Variables below are needed for dry deposition (Sun, Oct 2018):
                if (single_site_flag && use_site_obs_flag && (FLUXNET_flag || (using_site_met_flag && replaced_map_precip_flag))) {
                    if (FLUXNET_flag) {prec_liq = 0} else {prec_liq = PRECTOT[i,j,h]}
                } else {
                    # Derived from met. field
                    prec_liq = PRECTOT[i,j,h] - PRECSNO[i,j,h]
                }
                
                if (single_site_flag && use_site_obs_flag && FLUXNET_flag) {
                    d_snow = 0
                } else {
                    d_snow = SNODP[i,j,h]
                }
                ...
                # Surface pressure P_surf (Pa):
                # Pressure at zero-plane displacement height (d + z0m) where wind speed is extrapolated to zero P_disp (Pa):
                # At 2 m above displacement height P_2m:
                # At 10 m above displacement height P_10m:
                # Define T = theta at the surface.
                # Atmospheric pressure (Pa):
                if (single_site_flag && use_site_obs_flag && (FLUXNET_flag || (using_site_met_flag && replaced_map_ATMP_flag))) {
                    # Using measured site air pressure as P_disp, P_surf, P_2m and P_10m 
                    if (FLUXNET_flag) {
                        P_surf = ATMP[i,j,h]; P_disp = ATMP[i,j,h]; P_2m = ATMP[i,j,h]; P_10m = ATMP[i,j,h] 
                    } else {
                        P_surf = ATMP[h]; P_disp = ATMP[h]; P_2m = ATMP[h]; P_10m = ATMP[h] 
                    }
                } else {
                    P_surf = slp*exp(-Z_surf/Z_scale)
                    P_disp = slp*exp(-(Z_surf + Z_disp + Z_0m)/Z_scale)
                    P_2m = slp*exp(-(Z_surf + Z_disp + Z_0m + 2)/Z_scale)
                    P_10m = slp*exp(-(Z_surf + Z_disp + Z_0m + 10)/Z_scale)
                }
                
                # Atmospheric potential temperature (K):
                theta_2m = if (single_site_flag && use_site_obs_flag && FLUXNET_flag) T_2m + (g_E/c_p)*(Z_disp + Z_0m + 2) else T_2m*(P_surf/P_2m)^(R_da/c_p)
                # Vapor pressure (Pa):
                
                # For non-FLUXNET site measurements, use RH to deive e_2m
                if (single_site_flag && use_site_obs_flag && using_site_met_flag && exists('RH') && !is.na(RH[h])) {
                    # Using measured relative humidity to derive e_2m 
                    e_2m = RH[h] * f_esat(T_K = T_2m)
                } else {
                ...
                # Atmospheric potential temperature (K):
                # Define T = theta at the surface.
                theta_10m = if (single_site_flag && use_site_obs_flag && FLUXNET_flag) T_10m + (g_E/c_p)*(Z_disp + Z_0m + 10) else T_10m*(P_surf/P_10m)^(R_da/c_p)
                ...
                # In the first simulation step, declare new variables and read initial conditions from initial_map.nc
                # In other time steps, read simulation results from temp_data/xxx.RData of the last model-day.
                ...
                if (biogeochem_flag) {
                    if (current_date == start_date && h == 1) {
                        ...
                        BGC_variable_declaration_names = c("a_stem","a_leaf","a_stem_leafem","a_leaf_leafem","GDDT2m","GDDTsoil","GDD_mat","GDD_repr","GDD_emer",
                                                           "crop_live_flag", "peak_LAI_flag", "crop_plant_flag","leaf_emergence_flag","grain_fill_flag","harvest_flag",
                                                           "livestem_resv_C", "htop",
                                                           "day_of_planting","day_of_grain_filling","day_of_harvest", "day_since_crop_senes",
                                                           "CUO_grainfill")
                        
                        BGC_variable_declaration_value = c(rep(0, length(c("a_stem","a_leaf","a_stem_leafem","a_leaf_leafem","GDDT2m","GDDTsoil","GDD_mat","GDD_repr","GDD_emer"))),
                                                           rep(FALSE, length(c("crop_live_flag", "peak_LAI_flag", "crop_plant_flag","leaf_emergence_flag","grain_fill_flag","harvest_flag"))), 
                                                           rep(0, length(c("livestem_resv_C", "htop"))),
                                                           rep(NA, length(c("day_of_planting","day_of_grain_filling","day_of_harvest", "CUO_grainfill", "day_since_crop_senes"))))
                        ...
                        ###### Initialization by reading from CLM initialization map initial_map.nc
                        ...
                        BGC_variable_readin_names = c("a_stem","a_leaf","a_stem_leafem","a_leaf_leafem",
                                                      "GDDT2m","GDDTsoil","GDD_mat","GDD_repr","GDD_emer",
                                                      "crop_live_flag", "peak_LAI_flag", "crop_plant_flag","leaf_emergence_flag","grain_fill_flag","harvest_flag",
                                                      "day_of_planting","day_of_grain_filling","day_of_harvest", "day_since_crop_senes",
                                                      "leaf_C","fineroot_C","livestem_C","livestem_resv_C","deadstem_C","livecoarseroot_C","deadcoarseroot_C","grain_C", 
                                                      "LAI", "SAI", "htop")
                        
                        BGC_variable_readin_value = c(astem_PFT_lasthd_ijd[ipft], aleaf_PFT_lasthd_ijd[ipft], astem_leafem_PFT_lasthd_ijd[ipft], aleaf_leafem_PFT_lasthd_ijd[ipft], 
                                                      GDDT2m_PFT_lasthd_ijd[ipft], GDDTsoil_PFT_lasthd_ijd[ipft], GDDmat_PFT_lasthd_ijd[ipft], GDDrepr_PFT_lasthd_ijd[ipft], GDDemer_PFT_lasthd_ijd[ipft],
                                                      crop_live_flag_PFT_lasthd_ijd[ipft], peak_LAI_flag_lasthd_ijd[ipft], crop_plant_flag_PFT_lasthd_ijd[ipft], leaf_emergence_flag_lasthd_ijd[ipft], grain_fill_flag_lasthd_ijd[ipft], harvest_flag_lasthd_ijd[ipft], planting_jday_lasthd_ijd[ipft], grain_filling_jday_lasthd_ijd[ipft], harvesting_jday_lasthd_ijd[ipft], day_since_crop_senescence_lasthd_ijd[ipft],
                                                      leafC_PFT_lasthd_ijd[ipft], frootC_PFT_lasthd_ijd[ipft], livestemC_PFT_lasthd_ijd[ipft], livestemC_resv_PFT_lasthd_ijd[ipft], deadstemC_PFT_lasthd_ijd[ipft], livecrootC_PFT_lasthd_ijd[ipft], deadcrootC_PFT_lasthd_ijd[ipft], grainC_PFT_lasthd_ijd[ipft], 
                                                      tlai_PFT_lasthd_ijd[ipft], tsai_PFT_lasthd_ijd[ipft], htop_PFT_lasthd_ijd[ipft])
                ...
                vpd = if (single_site_flag && using_site_met_flag && use_site_obs_flag && replaced_map_VPD_flag) {
                    # Site-measurements
                    VPD[h]
                } else {
                    # Derived from reanalysis product
                    max(0, (f_esat(T_a) - e_a)/1000)
                }
                ...
                if (single_site_flag && use_site_obs_flag && 
                    (FLUXNET_flag || (using_site_met_flag && (replaced_map_GWETBULK_flag || (replaced_map_GWETTOP_flag && replaced_map_GWETROOT_flag))))) {
            
                    # Site-measured soil wetness data
                    if (FLUXNET_flag) {
                        soil_wetness_top = GWETTOP[i,j,h] / theta_sat_top / 100
                    } else if (replaced_map_GWETBULK_flag) {
                        # 1 layer of available SWC measurements
                        soil_wetness_top = GWETBULK[h] / theta_sat
                    } else if (replaced_map_GWETTOP_flag && replaced_map_GWETROOT_flag) {
                        # 2 layers of available SWC measurements
                        soil_wetness_top = GWETTOP[i,j,h] / theta_sat_top
                        soil_wetness_root = GWETROOT[i,j,h] / theta_sat_bottom
                    }
                } else {
                    # Soil wetness data from MERRA2/GEOS-FP maps
                ...
                if (biogeochem_flag) {
                    if (single_site_flag && using_site_met_flag && use_site_obs_flag && any(replace_TS_flag_array) && replaced_map_TS_flag) {
                        # TS from valid site-measured values
                        TS_var_name = c('TSOIL1','TSOIL2','TSOIL3','TSOIL4','TSOIL5')
                        T_soil_array_input = c()
                        for (ts_index in 1:sum(replace_TS_flag_array)) {
                            # extract the hourly TS from the TS arrays based on the number of available TS layers
                            T_soil_array_input = c(T_soil_array_input, get(TS_var_name[ts_index])[i,j,h])
                        }
                        
                        # Remove layers that don't have a TS value
                        T_soil_array_input = T_soil_array_input[!is.na(T_soil_array_input)]
                        
                    } else {
                        # TS from data maps
                        if (T_soil_source == 'MERRA2') {
                            T_soil_array_input =  c(TSOIL1[i,j,h], TSOIL2[i,j,h], TSOIL3[i,j,h], TSOIL4[i,j,h], TSOIL5[i,j,h])
                        } else if (T_soil_source == 'custom') {
                ...
                if (!O3_fixed_flag) {
                    if (use_site_obs_flag && using_site_o3_flag) {
                        # O3 read from dataframe
                        O3_conc = O3_hourly
                    } else {
                        # O3 read from .nc map
                        O3_conc = O3_hourly[i,j,(n_day_whole-1)*24+h]
                    }
                }
                ...
                if (!O3_damage_flag) {
                    ...
                    if (h == 1 & (d == 1 & !continue_flag)) {
                        # First day and hour of the simulation, initialize CUO_prev
                        CUO_prev_sun = 0
                        CUO_prev_sha = 0
                        CUO_prev_can = 0
                    } else if (h == 1 & (d > 1 | continue_flag)) {
                        # Read temp data for the first hour of a simulation day
                ...
                if (!biogeochem_flag) {
                    ...
                    if (current_date != start_date) {
                        # When LAI is calculated, new LAI is calculated at h == 24, therefore LAI_prev and LAI are taken from temp_data of d = n - 2 and d = n - 1 respectively.
                ...
                if (cos_SZA <= 0) {
                    ...
                    # Absorption of direct beam and diffuse radiation by sunlit and shaded leaves (0-1):
                    # Handle the exceptions in crop simulations when LAI and SAI are 0 before planting. The outputs are NaN. (Pang, Jun 2019)
                    ...
                    # Sunlit leaf area index:
                    # Handle the exception in the crop model. When the crops are not planted, LAI and SAI are zero. (Pang, Jun 2019)
                ...
                if (any(PFT_df$PFT_description[ipft] == c('c3_irrigated',
                                                          'irrigated_corn',
                                                          'irrigated_spring_temperate_cereal',
                                                          'irrigated_winter_temperate_cereal',
                                                          'irrigated_soybean'))) {
                    # No water stress for irrigated PFTs.
                    beta_t = 1
                } else {
                    # theta_sat_bulk; theta_sat_bulk_top; theta_sat_bottom
                    # b_psi_bulk; b_psi_bulk_top: b_psi_bulk_bottom
                    # psi_sat_bulk; psi_sat_bulk_top; psi_sat_bulk_bottom
                    # at [i,j] are replaced by site value already... 
                    if (single_site_flag && use_site_obs_flag && using_site_met_flag && 
                        (replaced_map_GWETBULK_flag || (replaced_map_GWETTOP_flag && replaced_map_GWETROOT_flag)) ) {
                        if (replaced_map_GWETBULK_flag) {
                            # Using bulk soil layer module to calculate beta_t
                            beta_t = f_water_stress(soil_wetness = soil_wetness_top, # from soil_wetness_top = GWETBULK[h] / theta_sat_top
                                                    soil_wetness_bottom = NULL,
                                                    soil_wetness_top = NULL,
                                                    root_frac_in_top = NA,
                                                    theta_w = theta_w, theta_sat = theta_sat,
                                                    psi_sat = psi_sat, b_psi = b_psi,
                                                    psi_c = psi_c, psi_o = psi_o,
                                                    psi_sat_top = NULL, b_psi_top = NULL,
                                                    psi_sat_bottom = NULL, b_psi_bottom = NULL,
                                                    multilayer = 'bulk')
                        } else if (replaced_map_GWETTOP_flag && replaced_map_GWETROOT_flag) {
                            beta_t = f_water_stress(soil_wetness = NULL,
                                                    soil_wetness_bottom = soil_wetness_root, 
                                                    soil_wetness_top = soil_wetness_top,    
                                                    root_frac_in_top = root_frac_in_top,
                                                    theta_w = NULL, theta_sat = NULL,
                                                    psi_sat = NULL, b_psi = NULL,
                                                    psi_c = psi_c, psi_o = psi_o,
                                                    psi_sat_top = psi_sat_top, b_psi_top = b_psi_top, 
                                                    psi_sat_bottom = psi_sat_bottom, b_psi_bottom = b_psi_bottom, 
                                                    multilayer = 'two-layer')
                        } else {
                            # reserve for beta_t calculation for more than 2 layers of SWC site measurements
                        }
                    } else {
                        # Calculate beta_t with met. field SWC (2 layer by default)
                ...
                ####################################################################
                # Start of the crop module
                # Calculations of carbon-vegetation dynamic
                if (biogeochem_flag) {
                    coarseroot_cn_ratio = liveWood_cn
                    grain_cn_ratio = grain_cn
                    # Change of plant C:N ratio at different phenological stages for crops
                    if (!crop_flag || (crop_flag && !grain_fill_flag)) {
                        # non-crop, and crops before the reproductive stage
                        leaf_cn_ratio = leaf_cn
                        fineroot_cn_ratio = froot_cn
                        stem_cn_ratio = liveWood_cn
                    } else if (crop_flag && grain_fill_flag) {
                        # crops after the reproductive stage
                    ...
                    root_frac_Tsoil_input = if (single_site_flag && use_site_obs_flag && using_site_met_flag && replaced_map_TS_flag) {
                        # MR root fraction weight calculated with specified depth from site TS measurement
                        root_frac_site_Tsoil_pft
                    } else {
                        # MR root fraction weight calculated with specified depth from MERRA2 TS maps
                        root_frac_map_Tsoil_pft
                    }

                    if (main_resp_scheme == 'CLM4.5') {
                        mr_fluxes = f_maintenance_respiration_fluxes_CLM45(root_frac_array = root_frac_Tsoil_input, 
                                                                           T_soil_array = T_soil_array_input, 
                                                                           T_2M = T_2m, 
                                                                           leaf_N = leaf_C / leaf_cn_ratio, 
                                                                           # special case for JULES-crop: livestemC = livestemC + livestemC_resv
                                                                           livestem_N = if (managed_crop_flag && crop_C_partit_scheme == 'JULES') {(livestem_C + livestem_resv_C)/ stem_cn_ratio} else {livestem_C / stem_cn_ratio}, 
                                                                           livecoarseroot_N = livecoarseroot_C / coarseroot_cn_ratio, 
                                                                           fineroot_N = fineroot_C / fineroot_cn_ratio, 
                                                                           grain_N = grain_C / grain_cn_ratio)
                    } else if (main_resp_scheme == 'custom') {
                        # mr_fluxes = f_maintenance_respiration_fluxes_custom(...)
                    }
                    ...
                    # Plant phonology, allocation of assimilated carbon, plant-carbon budgets, and plant physiological changes 
                    # Time step: 1 day
                    if (h == 24/dt_hr) {
                        ...
                        if (evergreen_flag) {
                            ...
                            # Phenology for crop-type PFT (excluding generic crops 'c3_crop' and 'c3_irrigated')
                            crop_phenology = f_crop_phenology(T_10_d = T_10d, T_min_10_d = Tmin_10d, T_soil = T_soil_daily, T2m = T_daily,
                                                              is_maize = maize_flag, is_springwheat = springwheat_flag, is_winterwheat = winterwheat_flag, is_soybean = soybean_flag,
                                                              leafC = leaf_C, livestemC = livestem_C, livestemresvC = livestem_resv_C, finerootC = fineroot_C, grainC = grain_C, LAI = LAI, SAI = SAI,
                                                              ...
                                                              prescribed_planting_date_readin = if (crop_planting_date_method != 'CLM4.5') {prescribed_planting_date} else {NULL}, planting_jday = day_of_planting, grain_filling_jday = day_of_grain_filling, harvest_jday = day_of_harvest, day_since_crop_senescence = day_since_crop_senes,
                            ...
                            simulateIJ_crop_phenology_variable_name = c("crop_emergence_flux", "leaf_C_loss_flux", "grain_C_loss_flux", "livestem_C_loss_flux", "livestem_resv_C_loss_flux", "fineroot_C_loss_flux",
                                                                        "crop_live_flag", "crop_plant_flag", "leaf_emergence_flag", "grain_fill_flag", "harvest_flag", "day_since_crop_senes",
                                                                        "day_of_planting", "day_of_grain_filling", "day_of_harvest",
                                                                        "GDDT2m", "GDDTsoil", "GDD_mat", "GDD_repr", "GDD_emer", "DVI",
                                                                        "leaf_C", "grain_C", "fineroot_C", "livestem_C", "livestem_resv_C",
                                                                        "LAI","SAI")
                            f_crop_phenology_output_listvariable_name = c("seedC_to_leafC_flux", "leafC_loss_flux", "grainC_loss_flux", "livestemC_loss_flux", "livestem_resvC_loss_flux", "finerootC_loss_flux",
                                                                          "croplive_flag", "cropplant_flag", "leafemergence_flag", "grainfill_flag", "har_flag", "day_since_crop_senescence",
                                                                          "planting_julianday", "grain_filling_julianday", "harvesting_julianday",
                                                                          "GDD_T2m", "GDD_Tsoil", "GDDmat", "GDDrepr", "GDDemer", "DVI",
                                                                          "leafC", "grainC", "finerootC", "livestemC", "livestemresvC",
                                                                          "LAI_out","SAI_out")
                            ...
                            crop_allocation_fluxes = f_crop_allocation_fluxes(A_can_umolm2s1 = An_daily, mr_total = mr_daily, gr_fraction = if (managed_crop_flag) 0.25 else 0.3, 
                                                                              is_maize = maize_flag, is_springwheat = springwheat_flag, is_winterwheat = winterwheat_flag, is_soybean = soybean_flag,
                            ...
                            simulateIJ_crop_allocation_variable_name = c("leaf_C_alloc_flux","fineroot_C_alloc_flux","livestem_C_alloc_flux","livestem_resv_C_alloc_flux","deadstem_C_alloc_flux","livecoarseroot_C_alloc_flux","deadcoarseroot_C_alloc_flux","grain_C_alloc_flux",
                                                                         "a_stem","a_leaf","a_root","a_repr","a_stem_leafem","a_leaf_leafem")
                            f_crop_allocation_output_listvariable_name = c("leaf_carbon_partitioning_flux_gCm2s1","fineroot_carbon_partitioning_flux_gCm2s1","livestem_carbon_partitioning_flux_gCm2s1","livestem_resv_carbon_partitioning_flux_gCm2s1","deadstem_carbon_partitioning_flux_gCm2s1","livecoarseroot_carbon_partitioning_flux_gCm2s1","deadcoarseroot_carbon_partitioning_flux_gCm2s1","grain_carbon_partitioning_flux_gCm2s1",
                        ...
                        } # End of the calculation of phenological C fluxes
                        
                        # Updating carbon pool budgets
                        # Some of the C pools are always zero: reserved for natural vegetation
                        plant_Cpool_budgets = f_Cpool_budgets(is_crop = managed_crop_flag, is_evergreen = evergreen_flag, is_stress_decid = stress_decid_flag, is_season_decid = season_decid_flag, 
                                                              is_maize = maize_flag, is_soybean = soybean_flag, is_springwheat = springwheat_flag, is_winterwheat = winterwheat_flag,
                                                              grain_filling_flag = if (managed_crop_flag) crop_phenology$grainfill_flag else NA, 
                                                              harvesting_flag = if (managed_crop_flag) crop_phenology$har_flag else NA, 
                                                              DVI = if (managed_crop_flag) crop_phenology$DVI else NA,
                                                              astem = if (managed_crop_flag && crop_transloc_scheme == 'JULES') {crop_allocation_fluxes$astem} else {NA},
                                                              ...
                                                              livestem_resv_C_prev = livestem_resv_C, livestem_resv_C_biomass_partitioning_flux_gCm2s1 = livestem_resv_C_alloc_flux, livestem_resv_C_loss_flux_gCm2s1 = livestem_resv_C_loss_flux,
                        ...
                        livestem_resv_C = plant_Cpool_budgets$livestem_resv_C_new
                        ...
                        vegetation_structure = f_vegetation_structure(is_crop = crop_flag, is_shrub = shrub_flag, is_evergreen = evergreen_flag, is_stress_decid = stress_decid_flag, is_season_decid = season_decid_flag, is_woody = woody_flag,
                                                                      is_managed_crop = managed_crop_flag, is_maize = maize_flag, is_soybean = soybean_flag, 
                                                                      slatop = sla_top, dsladlai = dsla_dlai, laimx = LAI_max, ztopmx = ztopmax,
                                                                      leafC = leaf_C, deadstemC = deadstem_C, 
                                                                      livestemC = if (managed_crop_flag && crop_C_partit_scheme == 'JULES') {livestem_C + livestem_resv_C} else {livestem_C},
                                                                      h_top = htop, tlai = LAI, tsai = SAI, 
                                                                      peak_lai_flag = peak_LAI_flag, harvesting_flag = harvest_flag, crop_living_flag = crop_live_flag, crop_DVI = if (managed_crop_flag) {crop_phenology$DVI} else {NA})
                        
                    } # End of the simulation of daily vegetation carbon fluxes and budgets.
                } # End of the simulation of vegetation fluxes and budgets including maintenance respiration.
                ...
                # Additional outputs in the crop model
                if (biogeochem_flag) {
                    if (evergreen_flag) {
                    ...
                    output_assign_df = rbind.data.frame(output_assign_df,
                                                        ...
                                                        c('mr_leaf_PFT_hist_ijd','mr_fluxes$mr_leaf'),
                                                        c('mr_fineroot_PFT_hist_ijd','mr_fluxes$mr_fineroot'),
                                                        c('mr_livestem_PFT_hist_ijd','mr_fluxes$mr_livestem'),
                                                        c('mr_livecoarseroot_PFT_hist_ijd','mr_fluxes$mr_coarseroot'),
                                                        c('mr_grain_PFT_hist_ijd','mr_fluxes$mr_grain'),
                                                        c('mr_total_PFT_hist_ijd','mr_fluxes$mr_total'),
                                                        # Vegetation physiology
                                                        c('LAI_PFT_hist_ijd',if (!using_site_LAI_flag) {'vegetation_structure$tlai'} else {'LAI_today'}),
                                                        ...
                                                        c('livestemC_resv_PFT_hist_ijd', 'plant_Cpool_budgets$livestem_resv_C'),
                                                        ...
                                                        c('day_since_crop_senescence_PFT_hist_ijd', ifelse(test = ipft < 18, yes = as.character(NA), no = 'crop_phenology$day_since_crop_senescence')),
                                                        # Carbon allocation coefficients (CLM4.5)
                ...
                for (ivar in seq_along(var_name$variable_name)) {
                    ...
                    # Assigning PFT_daily variables only at the last time step of a simulation day, hourly variable types are not affected
                ...
                # Essential outputs needed for the next time step in the crop model
                if (biogeochem_flag && h == 24/dt_hr) {
                    BGC_temp_var_vec = c('LAI_PFT_hist_ijd', 'SAI_PFT_hist_ijd', 'htop_PFT_hist_ijd',
                                         'leafC_PFT_hist_ijd', 'finerootC_PFT_hist_ijd', 'livestemC_PFT_hist_ijd', 'livestemC_resv_PFT_hist_ijd', 'deadstemC_PFT_hist_ijd', 'livecoarserootC_PFT_hist_ijd', 'deadcoarserootC_PFT_hist_ijd', 'grainC_PFT_hist_ijd',
                                         'GDDT2m_PFT_hist_ijd', 'GDDTsoil_PFT_hist_ijd', 'GDDmat_PFT_hist_ijd', 'GDDemer_PFT_hist_ijd', 'GDDrepr_PFT_hist_ijd',
                                         'crop_live_flag_PFT_hist_ijd', 'crop_plant_flag_PFT_hist_ijd', 'leaf_emergence_flag_PFT_hist_ijd', 'grain_fill_flag_PFT_hist_ijd', 'harvest_flag_PFT_hist_ijd', 'peak_LAI_flag_PFT_hist_ijd',
                                         'day_of_planting_PFT_hist_ijd', 'day_of_grain_filling_PFT_hist_ijd', 'day_of_harvesting_PFT_hist_ijd', 'day_since_crop_senescence_PFT_hist_ijd',
                                         'astem_PFT_hist_ijd', 'aleaf_PFT_hist_ijd', 'astem_leafem_PFT_hist_ijd', 'aleaf_leafem_PFT_hist_ijd')
            ...
            } # End of simulation for a hour.
        }  # else (!low_PFT_frac)
    } # End of simulation for a PFT.
    ...
    if (biogeochem_flag) {
        ...
        temp_env$livestemC_resv_PFT_lasthd_ijd = livestemC_resv_PFT_hist_ijd
        ...
        # GDD, phenology and day of planting/harvesting
        ...
        temp_env$day_since_crop_senescence_lasthd_ijd = day_since_crop_senescence_PFT_hist_ijd
        # Biomass allocation coefficients
