...
# Parameters used in crop simulation (TEMIR version 2.0)
if (biogeochem_flag){
    ...
    # Prescribed maximum LAI allowed for crops (can be removed with 'limit_crop_LAI_flag').
    laimx = ncvar_get(nc, 'laimx')
    # Leaf longevity (same as above)
    leaf_long[18:19] = 1/6                     # Adjust maize leaf longetivity to ~60 days (Wolfe et al. 1988, Agronomy Journal) instead of 
    leaf_long[24:25] = 1/15                    # Adjust soybean leaf longetivity to ~25 days (Miyaji 1984, New Phytologist)
    ...
    # Default: Maize 165 days, Spring wheat 150 days, Winter wheat: 265 days, Soybean 150 days
    # Some FLUXNET maize sites have growing season longer than that - add 30 days uniformly
    mxmat = mxmat + 30
    ...
    # Earliest/lastest possible planting day in the Northern Hemisphere (Day of year)
    ...
    # Earliest/lastest possible planting day in the Southern Hemisphere (Day of year)
...
# New CLM surface input - include crop coverage in the tropical region
...
# Root fraction calculated with the soil layer depth of soil temperature measurements
if (biogeochem_flag) {
   
  # Single-site soil temperature measurements
  if (using_site_met_flag && any(replace_TS_flag_array) == TRUE) {
     if (length(site_TS_meas_depth) == 1) {
       # only 1 soil temperature layer
       root_frac_site_Tsoil = array(1, dim = length(pftname))
     } else {
       root_frac_site_Tsoil = matrix(0, nrow = length(pftname), ncol = length(site_TS_meas_depth))
       z_hi_site_Tsoil = c(0, site_TS_meas_depth)
       for (i in 1:length(site_TS_meas_depth)) {
         if (i < length(site_TS_meas_depth)) {
           root_frac_site_Tsoil[,i] = 0.5*(exp(-roota_par*z_hi_site_Tsoil[i]) + exp(-rootb_par*z_hi_site_Tsoil[i]) - exp(-roota_par*z_hi_site_Tsoil[i+1]) - exp(-rootb_par*z_hi_site_Tsoil[i+1]))
         } else {
           root_frac_site_Tsoil[,i] = 0.5*(exp(-roota_par*z_hi_site_Tsoil[i]) + exp(-rootb_par*z_hi_site_Tsoil[i]))
         }
       }
     }
  } 
  
  # Mapped TS data (e.g., MERRA2), required regardless using site-measured TS or not.
  z_hi_map_Tsoil = c(0, map_TS_meas_depth)
  if (length(map_TS_meas_depth) == 1) {
    # only 1 soil temperature layer
    root_frac_map_Tsoil = array(1, dim = length(pftname))
  } else {
    root_frac_map_Tsoil = matrix(0, nrow = length(pftname), ncol = length(map_TS_meas_depth))
    for (i in 1:length(map_TS_meas_depth)) {
      if (i < length(map_TS_meas_depth)) {
        root_frac_map_Tsoil[,i] = 0.5*(exp(-roota_par*z_hi_map_Tsoil[i]) + exp(-rootb_par*z_hi_map_Tsoil[i]) - exp(-roota_par*z_hi_map_Tsoil[i+1]) - exp(-rootb_par*z_hi_map_Tsoil[i+1]))
      } else {
        root_frac_map_Tsoil[,i] = 0.5*(exp(-roota_par*z_hi_map_Tsoil[i]) + exp(-rootb_par*z_hi_map_Tsoil[i]))
      }
    } 
  }
}
...
if (biogeochem_flag && crop_planting_date_method == 'prescribed-map') {
    
    if (dlon < 0.5 || dlat < 0.5) {
        print('The Sack et al. crop calender has a resolution of 0.5x0.5. Thw simulation resolution is higher than the data resolution, regridding is not required.')
    }
    
    if (dlon == 0.5 && dlat == 0.5) {
        # Simulation resolution is the same as the data, can be read in directly from .nc
        filename = crop_calender_ncname
        ...
        if (!file.exists(filename)) {
            ...
            nc_filename = crop_calender_ncname
...
if (biogeochem_flag) {
    if (GDDmat_method == 'prescribed-map') {
        if (dlon == 360/540 && dlat == 0.5) {
            ...
            warning('The GDDmat map has a resolution of 0.667x0.5. The simulation resolution is higher than the data resolution. Regridding is not needed.')
        } else {
...
if (O3_damage_flag & !O3_fixed_flag & (use_site_obs_flag && !using_site_o3_flag)) {
...
if (biogeochem_flag) {
  ...
  # storage C pools x7 for deciduous (reserved for natural vegetations)
