...
f_ozone_impact = function(O3_conc, g_s, g_b, g_ah, P_atm=101325, theta_atm=298.15, met_cond=TRUE, scheme='Lombardozzi', dt=3600, LAI=0.5, LAI_prev=0.5, evergreen=FALSE, leaf_long=1e3, CUO_prev=0, sensitivity='high', plant_group='broadleaf') {
	...
	if (sensitivity == 'custom-soybean') {
	    # parameter derived for soybean
	    # new parameters SoyFACE Fcrit 3-4 nmol O3 m-2 s-1 (take 4 nmol O3 m-2 s-1)
	    O3_flux_th = 4
	} else if (sensitivity == 'custom-maize') {
	    # parameter derived for maize Fcrit 2-3 nmol O3 m-2 s-1 (take 3 nmol O3 m-2 s-1)
	    O3_flux_th = 3
	} else {
	    # default threshold in the Lombardozzi scheme
	    O3_flux_th = 0.8
	}
        ...
        if (sensitivity == 'high') {
            ...
            grass_gs_int = 0.7511
            grass_gs_slope = 0
        } else if (sensitivity == 'custom-soybean') {
	    # parameters derived for soybean from soyFACE (Fcrit = 4)
	    # An_slope: -0.0223 (cultivar range -0.0174 to -0.0289)
	    # gs_slope: -0.0199 (cultivar range 0 to -0.0300)
            grass_An_int = 1
            grass_An_slope = -0.0174   # low -0.0174; mean: -0.0223; high: -0.0289
	    grass_gs_int = 1
            grass_gs_slope = -0.0300   # low 0; mean: -0.0199; high: -0.0300
        } else if (sensitivity == 'custom-maize') {
	    # parameters derived from Peng et al. 2020 and Singh et al. 2023 (Fcrit = 3)
	    # An_slope: -0.10 +- 0.04  (Peng: -0.085+-0.013; Singh: -0.124+-0.029), based on relative Asat vs. CUO
	    # An_slope (Peng): -0.05 (calibrated to rel. total biomass)
	    # An_slope (Peng): -0.03 (calibrated to rel. yield)
	    
	    # gs_slope: -0.05 +- 0.05 (Peng: -0.04+-0.01 (forced intercept) or 0; Singh: -0.10+-0.03)
	    grass_An_int = 1
            grass_An_slope = -0.030  # low: -0.06; medium: -0.10; high: -0.14
	    grass_gs_int = 1 
	    grass_gs_slope = -0.050   # low: 0;  medium: -0.05;  high:  -0.1
	}
        ...
        # Testing Felix's parameter
        if (sensitivity == 'custom-soybean') {O3_flux_th = 15}
    	O3_flux_crit = max(c(0, (O3_flux - O3_flux_th)), na.rm=TRUE)
...
f_canopy_photosyn = function(c_a, e_a, phi_sun, phi_sha, T_v=298.15, P_atm=101325, theta_atm=298.15, C3_plant=TRUE, V_cmax25_0, Phi_PSII=0.85, Theta_PSII=0.70, beta_t=1, acclimation=TRUE, T_10d=298.15, lat=0, decl=0, colimit=TRUE, LAI, LAI_sun, K_n=0.30, K_b=0.50, O3_damage=FALSE, O3_conc=NULL, scheme='Lombardozzi', gs_scheme = 'FBB', dt=3600, LAI_prev=NULL, evergreen=FALSE, leaf_long=1e3, CUO_prev_sun=0, CUO_prev_sha=0, CUO_prev_can = 0, sensitivity='high', plant_group='broadleaf', g_ah=NULL, biogeochem=FALSE, leaf_N_conc=NULL, u_leaf, d_leaf=0.04, met_cond=FALSE, tol=1e-3, g1_med=NULL) {
   ...
   # *** Now modified such that "u_atm" is not needed anymore because we directly read in "g_ah" (aerodynamic conductance for heat, umol m^-2 s^-1 or m s^-1) as input. (Tai, Feb 2019)
    ...
    CUO_can_change = ifelse (test = LAI_sha + LAI_sun == 0 | is.na(LAI_sha + LAI_sun), yes = 0,
    ...
    CUO_can = ifelse (test = LAI_sha + LAI_sun == 0 | is.na(LAI_sha + LAI_sun), yes = 0,
...
f_water_stress = function(soil_wetness=NULL, soil_wetness_top=NULL, soil_wetness_bottom=NULL, theta_w=NULL, theta_top = NULL, theta_bottom = NULL, theta_sat=NULL, psi_sat, b_psi, psi_sat_top, b_psi_top, psi_sat_bottom, b_psi_bottom, psi_c, psi_o, root_frac_in_top, multilayer) {
    ...
    if (multilayer == 'multilayer') {
        ...
        if (is.null(soil_wetness_bottom)) soil_wetness_bottom = theta_bottom/theta_sat
        if (is.null(soil_wetness_top)) soil_wetness_top = theta_top/theta_sat
