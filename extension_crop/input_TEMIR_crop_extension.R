...
# Initial data provides the starting values of LAI, SAI, and carbon pools of different PFTs 
...
initial_data_dir = '/users/b146986/TEMIR/TEMIR_inputs/initial_data/'
...
# soilT_data_dir = paste0(TEMIR_dir,'TEMIR_inputs/soilT/')
...
# Specifiy the depth of the bottom of the soil layer of the soil temperature input (in m) for the calculation of root fraction
...
if (crop_planting_date_method == 'prescribed-map') {
    ...
    crop_calender_map_dir = '/users/b146986/TEMIR/TEMIR_inputs/crop_calender/' 
...
# Specifiy the prescirbed planting date in day of year
...
if (any(GDDmat_method == c('CLM4.5', 'prescribed-map'))) {
    # GDDmat_map_dir = paste0(TEMIR_dir, 'TEMIR_inputs/GDDmat_map/')
    GDDmat_map_dir = '/users/b146986/TEMIR/TEMIR_inputs/GDDmat_map/'
}

# Specify a GDDmat value for single site and single PFT simulation.
if (GDDmat_method == 'prescribed-site') {
    prescribed_GDDmat = 1500 # Growing degree days
}
...
if (GDDrepr_method == 'prescribed-site') {
    prescribed_GDDrepr = 1050 # Growing degree days
}
...
if (GDDemer_method == 'prescribed-site') {
    prescribed_GDDemer = 120 # Growing degree days
}

## How is the increase of growing degree day GDD calculated? ('CLM4.5', 'custom')
# 'CLM4.5' (default): the increase of GDD is calculated using the parameters from CLM4.5
...
# 'JULES' (default): based on the translocation scheme for crops in JULES-crop. This scheme assumes carbon loss during leaf senescence goes to the grain during the reproductive stage. Noted that the DVI (phenology) requirement is reduced from 1.5 (in JULES) to 1.0.
...
# DVI: -1 (just planted); 0 (just emerged); 1 (just reaching the reproductive stage); 2 (mature and harvested).
# leaf senescence starts at DVI = 1.5 in JULES; DVI = 1.0 in CLM4.5 (equivalent).
if (crop_leaf_sen_scheme == 'custom') {
    leaf_senescence_DVI = 1.0 # unitless
}
...
# if (crop_planting_date_method == 'prescribed-map' && !dir.exists(crop_calender_map_dir)) stop('Directory for crop calender maps does not exist!')	
